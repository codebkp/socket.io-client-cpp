...
client_impl::client_impl()
    : m_con_state(con_closed), m_ping_interval(0), m_ping_timeout(0),
...
void client_impl::connect_impl(const string &uri, const string &queryString) {
  do {
    ...
    if (m_sid.size() == 0) {
      ...
      ss << uo.get_host() << ":" << uo.get_port()
         << "/socket.io/?EIO=4&transport=websocket&sid=" << m_sid
         << "&t=" << time(NULL) << queryString;
    }
    lib::error_code ec;
...
void client_impl::stop() { m_client.stop(); }
...
unsigned client_impl::next_delay() const {
  ...
  unsigned reconn_made =
      min<unsigned>(m_reconn_made, 32); // protect the pow result to be too big.
...
void client_impl::on_handshake(message::ptr const &message) {
  if (message && message->get_flag() == message::flag_object) {
    const object_message *obj_ptr =
        static_cast<object_message *>(message.get());
...
#if SIO_TLS
#ifdef SIO_EMBED_CACERTS
// Boost is buggy and doesn't correctly load certs from memory (from file
// works). This loads from memory ourselves.
void add_certificate_authority(SSL_CTX *ctx, const void *certs_data,
                               size_t certs_size) {
  ...
  char empty_string[1] = {
      '\0'}; // Can't be a literal, OpenSSL wants a NON-const empty string!
  STACK_OF(X509_INFO) *inf =
      PEM_X509_INFO_read_bio(bio, nullptr, nullptr, empty_string);
  if (!inf)
    goto error_inf;
